...
export const actions = {
  ...
  async addCartItem({ commit, dispatch, state }, item) {
    ...
    try {
      ...
      // Make Swell API call
      await dispatch('initializeCustomer', {});
      ...
      if (state.notification !== null) {
        ...
        window.setTimeout(() => {
          dispatch('showNotification', {
            message: `Agregamos ${item.quantity} producto(s) al carrito`,
            type: 'product',
            isSticky: true,
          });
        }, 200);
   ...
   * @param {object} fields - Cart item fields to update in key: value format
   */
  async updateShipping({ commit, dispatch, state }, data) {
    if (state.cartIsUpdating) return;
    commit('setState', { key: 'cartIsUpdating', value: true });
    ...
    try {
      ...
      const cart = await this.$swell.cart.update({
        shipping: data.dataAddress,
      });
  ...
  async updateShippingMethod({ commit, dispatch, state }, data) {
    if (state.cartIsUpdating) return;
    commit('setState', { key: 'cartIsUpdating', value: true });
  ...
  closeCart({ commit, dispatch, state }) {
    try {
      commit('setState', { key: 'cart', value: null });
    } catch (err) {
      dispatch('handleError', err);
    }
  },
