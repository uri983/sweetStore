<template>
  <footer
    :class="{
      'border-primary-darker bg-white': background === 'dark',
    ...
    <div class="container py-16 text-center lg:flex lg:flex-row lg:text-left">
        ...
        >
          ...
          >
            <a
              :href="link.url"
              target="_blank"
              :title="`Siguenos en ${link.id}`"
              class="mx-2 block"
            >
              <BaseIcon class="text-sweetPrimary" :icon="`mdi:${link.id}`" />
            </a>
          </li>
        </ul>
      </div>
      ...
      <template v-if="footer.showEmailSignup">
        <div class="mx-auto max-w-96 lg:w-1/4 lg:flex-shrink-0">
          ...
          <EmailSignupForm :theme="'light'" class="mt-5" />
        </div>
      </template>
    </div>
    <!-- END Main footer -->
    ...
    >
      <div class="lg:flex-no-wrap container lg:flex lg:justify-between">
        ...
        <p class="my-3 text-sm">
          <span>©{{ currentYear }} The Sweet Spot.</span>
        </p>
        <!-- Secondary nav menu -->
...
export default {
  ...
  async fetch() {
    ...
    this.footer.showSocial = true;
    this.socialLinks = await $swell.settings.get('socialLinks', {});
    this.background = await $swell.settings.get('footer.background', 'light');
  },
