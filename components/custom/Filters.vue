<template>
  <div class="h-full shadow-2xl">
    <div
      ...
      class="overflow-hidden border-b border-primary-light bg-primary-lighter py-4 px-1 text-sm"
    >
    ...
    <div class="p-6">
      ...
      <!-- TODO: Sizes
          ...
          -->
      <button
        class="outline-none focus:outline-none mr-1 mb-1 w-full rounded bg-sweetPrimary px-8 py-3 text-center font-bold uppercase text-white shadow-md transition-all duration-150 ease-linear hover:shadow-lg active:bg-blue-600"
        @click="applyFilters"
        type="button"
      >
...
export default {
  ...
  created() {
    if (Object.keys(this.filterState).length) {
      ...
      if (this.localFilterState.price) {
        if (this.localFilterState.price.length > 0) {
