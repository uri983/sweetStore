<template>
  <!-- Panel -->
  <div class="right-0 h-full w-full bg-primary-lighter">
    <div class="h-full w-full overflow-y-scroll bg-primary-lighter">
      <!-- Header -->
      ...
      >
        <div v-if="false" class="container border-b border-primary-med py-6">
          <div class="flex">
            <input
              v-model="couponCode"
              type="text"
        ...
        <div class="container border-b border-primary-med py-6">
          ...
          <div v-show="cart.discountTotal" class="mb-1 flex justify-between">
...
export default {
  ...
  async fetch() {
    // Set component data
    const { $swell } = this;
    this.shopLink = await $swell.settings.get('/', '/');
  },
