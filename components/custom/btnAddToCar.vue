<template>
  <div>
    <button
      ...
      @click="interact"
      class="outline-none focus:outline-none mr-1 mb-1 w-full rounded bg-blue-200 py-2 text-xs uppercase text-blue-600 shadow transition-all duration-150 ease-linear hover:shadow-md active:bg-blue-200"
      type="button"
    >
    ...
    <div v-if="cartIsUpdating" class="relative px-4">
      <button
        ...
        type="button"
      >
        <span class="text-blue-600">{{ $t('Actualizando Carrito...') }}</span>
      </button>
    </div>
  </div>
</template>
