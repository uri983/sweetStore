<template>
  <div>
    ...
    <div v-if="order">
      <div class="container">
        <div class="mb-10 mt-8 border-b border-primary-med pb-6">
          <h2 class="text-2xl">
              ...
              <div>
                ...
                <div class="text-sm">
                  ...
                  <p class="pt-2 font-semibold">
                    {{ formatMoney(item.price_total, order.currency) }}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="flex pb-2">
            ...
            <span class="ml-auto">{{
              formatMoney(order.sub_total, order.currency)
            }}</span>
          </div>

          <div v-if="order.shipment_delivery" class="flex pb-2">
            <span>{{ $t('account.orders.id.shipping') }}</span>
            <span class="ml-auto">{{
              order.shipment_price === 0
                ? 'Free'
                : formatMoney(order.shipment_price, order.currency)
            }}</span>
          </div>

          <div v-if="order.item_discount > 0" class="flex pb-2">
            <span>{{ $t('account.orders.id.discountsAndCredits') }}</span>
            <span class="ml-auto"
              >-{{ formatMoney(order.item_discount, order.currency) }}</span
            >
          </div>

          <div v-if="order.tax_total > 0" class="flex pb-2">
      ...
      <!-- Delivery details -->
      <div class="container">
        <template v-if="order.shipment_delivery">
...
export default {
  ...
  async fetch() {
    ...
    const data = await axios.get('getOrder/' + this.$route.params.id);
    if (data.status === 200) {
  ...
  computed: {
    ...
    cardExpDate() {
      const mm = padStart(this.billing.card.exp_month, 2, '0');
      const yy = this.billing.card.exp_year.toString().slice(-2);
      return `${mm} / ${yy}`;
    },
