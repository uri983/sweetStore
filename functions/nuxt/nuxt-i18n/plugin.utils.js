...
export async function loadLanguageAsync(context, locale) {
  ...
  if (!i18n.loadedLanguages.includes(locale)) {
    const localeObject = options.normalizedLocales.find(
      (l) => l.code === locale,
    );
    if (localeObject) {
      ...
      if (file) {
        ...
        if (!messages) {
          try {
            // @ts-ignore
            const getter = await localeMessages[file]().then(
              (m) => m.default || m,
            );
            messages =
            ...
            console.error(
              formatMessage(
                `Failed loading async locale export: ${
        ...
        console.warn(
          formatMessage(`Could not find lang file for locale ${locale}`),
        );
      }
    } else {
      console.warn(
        formatMessage(
          `Attempted to load messages for non-existant locale code "${locale}"`,
        ),
      );
    }
  }
...
export function getDomainFromLocale(localeCode, req, { normalizedLocales }) {
  ...
  // eslint-disable-next-line no-console
  console.warn(
    formatMessage(`Could not find domain name for locale ${localeCode}`),
  );
}
      ...
      ...(vuex.syncRouteParams
        ? {
            localeRouteParams: ({ routeParams }) => {
              ...
              const paramsGetter = (locale) =>
                (routeParams && routeParams[locale]) || {};
              return paramsGetter;
            },
          }
        : {}),
...
export function validateRouteParams(routeParams, localeCodes) {
  ...
  for (const [key, value] of Object.entries(routeParams)) {
    if (!localeCodes.includes(key)) {
      ...
      console.warn(
        formatMessage(
          `Trying to set route params for key ${key} which is not a valid locale`,
        ),
      );
      ...
      console.warn(
        formatMessage(
          `Trying to set route params for locale ${key} with a non-object value`,
        ),
      );
    }
  }
 ...
 * @return {void}
 */
export function mergeAdditionalMessages(
  i18n,
  additionalMessages,
  localeCodes,
  onlyLocales,
