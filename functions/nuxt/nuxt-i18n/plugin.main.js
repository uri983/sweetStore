...
export default async (context) => {
  ...
  const { lazy } = options;
  const injectInNuxtState =
    lazy && (lazy === true || lazy.skipNuxtState !== true);
  ...
  const loadAndSetLocale = async (newLocale, { initialSetup = false } = {}) => {
    ...
    const localeOverride = app.i18n.onBeforeLanguageSwitch(
      oldLocale,
      newLocale,
      initialSetup,
      context,
      ...
      if (options.lazy) {
        ...
        if (i18nFallbackLocale) {
          ...
          if (Array.isArray(i18nFallbackLocale)) {
            localesToLoadPromises = i18nFallbackLocale.map((fbLocale) =>
              loadLanguageAsync(context, fbLocale),
            );
          } else if (typeof i18nFallbackLocale === 'object') {
            if (i18nFallbackLocale[newLocale]) {
              localesToLoadPromises = localesToLoadPromises.concat(
                i18nFallbackLocale[newLocale].map((fbLocale) =>
                  loadLanguageAsync(context, fbLocale),
                ),
              );
            }
            if (i18nFallbackLocale.default) {
              localesToLoadPromises = localesToLoadPromises.concat(
                i18nFallbackLocale.default.map((fbLocale) =>
                  loadLanguageAsync(context, fbLocale),
                ),
              );
            }
          } else if (newLocale !== i18nFallbackLocale) {
            localesToLoadPromises.push(
              loadLanguageAsync(context, i18nFallbackLocale),
            );
          }
        ...
        // Load all locales.
        await Promise.all(
          options.localeCodes.map((locale) =>
            loadLanguageAsync(context, locale),
          ),
        );
      }
    } else {
    ...
    const newLocaleProperties = options.normalizedLocales.find(
      (l) => l.code === newLocale,
    ) || { code: newLocale };
    ...
    if (
      ...
      (getLocaleFromRoute(route) !== newLocale ||
        (options.strategy === Constants.STRATEGIES.PREFIX_AND_DEFAULT &&
          newLocale === options.defaultLocale))
    ) {
      // The current route could be 404 in which case attempt to find matching route using the full path since
      // "switchLocalePath" can only find routes if the current route exists.
      const routePath =
        app.switchLocalePath(newLocale) ||
        app.localePath(route.fullPath, newLocale);
      if (
        routePath &&
        !isURLEqual(routePath, route.fullPath) &&
  ...
  const onNavigate = async (route) => {
    ...
    app.i18n.__baseUrl = resolveBaseUrl(
      options.baseUrl,
      context,
      app.i18n.locale,
  ...
  const doDetectBrowserLanguage = (route) => {
    ...
    if (options.strategy !== Constants.STRATEGIES.NO_PREFIX) {
      if (redirectOn === Constants.REDIRECT_ON_OPTIONS.ROOT) {
        ...
        if (
          !alwaysRedirect &&
          route.path.match(getLocalesRegex(options.localeCodes))
        ) {
          return '';
        }
      }
  ...
  const extendVueI18nInstance = (i18n) => {
    ...
    i18n.localeProperties = Vue.observable(
      klona(
        options.normalizedLocales.find((l) => l.code === i18n.locale) || {
          code: i18n.locale,
        },
      ),
    );
  ...
  const vueI18nOptions =
    typeof options.vueI18n === 'function'
      ? await options.vueI18n(context)
      : klona(options.vueI18n);
  ...
  let finalLocale = options.detectBrowserLanguage
    ? doDetectBrowserLanguage(route)
    : '';
