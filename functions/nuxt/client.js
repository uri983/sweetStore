    ...
    await Promise.all(
      Components.map(async (Component, i) => {
        ...
        if (hasAsyncData) {
          ...
          if (this.isPreview || spaFallback) {
            ...
            promise = this.fetchPayload(to.path)
              ...
              .catch((_err) =>
                promisify(Component.options.asyncData, app.context),
              ); // Fallback
          }
        ...
        if (!this.isPreview && !spaFallback) {
          ...
          promises.push(
            this.fetchPayload(to.path)
              .then((payload) => {
                payload.mutations.forEach((m) => {
                  this.$store.commit(m[0], m[1]);
                });
              })
              .catch((err) => null),
          );
...
async function mountApp(__app) {
  ...
  if (!NUXT.data && NUXT.serverRendered) {
    try {
      const payload = await _app.fetchPayload(
        NUXT.routePath || _app.context.route.path,
      );
