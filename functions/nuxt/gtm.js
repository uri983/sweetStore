...
function gtmServer(ctx, initialized) {
  ...
  ctx.beforeNuxtRender(() => {
    ...
    gtmScript.innerHTML = `window['${_layer}']=${JSON.stringify(events)};${
      gtmScript.innerHTML
    }`;

    if (inits.length) {
      gtmScript.innerHTML += `;${JSON.stringify(
        inits,
      )}.forEach(function(i){window._gtm_inject(i)})`;
    }

    const gtmIframe = ctx.app.head.noscript.find(
      (s) => s.hid == 'gtm-noscript',
    );
    const renderIframe = (id) =>
      `<iframe src="https://www.googletagmanager.com/ns.html?id=${id}&" height="0" width="0" style="display:none;visibility:hidden" title="gtm"></iframe>`;
    if (inits.length) {
...
export default function (ctx, inject) {
  ...
  const $gtm = process.client
    ? gtmClient(ctx, initialized)
    : gtmServer(ctx, initialized);
